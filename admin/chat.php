...
<script>
(function(){const list=document.querySelector('[data-chat-list]');if(!list)return;function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}function render(rows){if(!rows.length){list.innerHTML='<div class="card">No chat conversations yet.</div>';return;}list.innerHTML=rows.map(r=>`<article class="card conversation"><div><h2><a href="/admin/chat-thread.php?id=${Number(r.id)}">${esc(r.name||r.email||('Visitor conversation #'+r.id))}</a> ${Number(r.unread_admin_count||0)>0?`<span class="unread">${Number(r.unread_admin_count)} new</span>`:''}</h2><div class="meta">Status: <span class="status">${esc(r.status)}</span><br>Last message: ${esc(r.last_message||'No messages')}<br>Time on site: ${Number(r.total_time_on_site_seconds||0)}s · Updated: ${esc(r.updated_at)}</div></div><a class="btn" href="/admin/chat-thread.php?id=${Number(r.id)}">${Number(r.unread_admin_count||0)>0?'Accept Chat':'Open Thread'}</a></article>`).join('');}async function poll(){try{const r=await fetch('/admin/chat.php?ajax=1');const d=await r.json();if(d.ok)render(d.conversations||[]);}catch(e){}}setInterval(poll,5000);})();
</script>
