...
if (($_GET['ajax'] ?? '') === '1') {
    ...
    echo json_encode(['ok' => true, 'conversation' => $conversation, 'messages' => $messages]);
    exit;
}
...
<script>
(function(){
const appBase=<?= json_encode(app_base_path()) ?>;const appUrl=p=>appBase+p;const root=document.querySelector('[data-conversation-id]');if(!root)return;const id=root.dataset.conversationId;const canvas=root.querySelector('[data-chat-canvas]');const form=root.querySelector('[data-chat-reply-form]');const badge=document.querySelector('[data-thread-badge]');let lastHtml='';let lastVisitorKey='';let booted=false;let unread=0;let audioReady=false;
function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}
function visitorKey(messages){const visitor=[...(messages||[])].reverse().find(m=>m.sender_type==='visitor');return visitor?`${visitor.id||''}:${visitor.created_at}:${visitor.message}`:'';}
function notice(){if(!audioReady)return;try{const C=window.AudioContext||window.webkitAudioContext;const c=new C();const o=c.createOscillator();const g=c.createGain();o.frequency.value=780;g.gain.value=.04;o.connect(g);g.connect(c.destination);o.start();setTimeout(()=>{o.stop();c.close();},150);}catch(e){}}
function setBadge(value){unread=Math.max(0,value);if(!badge)return;badge.textContent=String(unread);badge.hidden=unread<=0;}
function render(messages){const html=messages.map(m=>`<div class="msg ${esc(m.sender_type)}"><div class="meta">${esc(m.sender_type)}${m.sender_name?' · '+esc(m.sender_name):''} · ${esc(m.created_at)}</div>${esc(m.message).replace(/\n/g,'<br>')}</div>`).join('');const key=visitorKey(messages);if(booted&&key&&key!==lastVisitorKey){setBadge(unread+1);notice();}if(key)lastVisitorKey=key;booted=true;if(html!==lastHtml){const nearBottom=canvas.scrollTop+canvas.clientHeight>=canvas.scrollHeight-80;canvas.innerHTML=html;lastHtml=html;if(nearBottom)canvas.scrollTop=canvas.scrollHeight;}}
...
form.addEventListener('submit',async e=>{e.preventDefault();setBadge(0);const fd=new FormData(form);const ta=form.querySelector('textarea');if(!ta.value.trim())return;try{const r=await fetch(appUrl('/admin/chat-thread.php?id='+encodeURIComponent(id)),{method:'POST',body:fd});const d=await r.json();if(d.ok){ta.value='';render(d.messages||[]);canvas.scrollTop=canvas.scrollHeight;}}catch(err){form.submit();}});
document.addEventListener('click',()=>{audioReady=true;},{once:true});document.addEventListener('keydown',()=>{audioReady=true;},{once:true});
