...
try {
    ...
    $items = [];
    foreach ($rows as $row) {
        $lastAt = $row['last_visitor_message_at'] ?: $row['last_message_at'] ?: $row['created_at'];
        $items[] = [
            'id' => (int)$row['id'],
            'label' => $row['name'] ?: $row['email'] ?: 'Visitor #' . $row['id'],
            'last_message' => de_strimwidth((string)($row['last_message'] ?: ''), 0, 120, '...'),
            'unread' => (int)$row['unread_admin_count'],
            'last_at' => $lastAt,
            'signature' => (int)$row['id'] . ':' . (int)$row['unread_admin_count'] . ':' . (string)$lastAt,
